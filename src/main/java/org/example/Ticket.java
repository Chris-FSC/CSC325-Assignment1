...
public abstract class Ticket {
    ...
    public int estimateResolutionHours(int complexityFactor) {
        if (complexityFactor < 1) throw new
                IllegalArgumentException("complexityFactor must be >= 1");
        int base = (6 - priority) * 2 + daysOpen;
    ...
    public int estimateResolutionHours(int complexityFactor, int afterHoursPenalty) {
        if (complexityFactor < 1) throw new
                IllegalArgumentException("complexityFactor must be >= 1");
        if (afterHoursPenalty < 0) throw new
                IllegalArgumentException("afterHoursPenalty must be >= 0");
        int base = (6 - priority) * 2 + daysOpen;
                double estimate = base + complexityFactor + afterHoursPenalty;
                    return (int) Math.max(1, estimate);
    }
